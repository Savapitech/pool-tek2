...
static const ArrayIteratorClass   _iteratorDescription = {
    {   /* Iterator struct */
        {   /* Class struct */
...
static void     Array_ctor(ArrayClass *this, va_list *args)
{
    ...
    if (this->_tab == NULL && this->_size)
        raise("Cannot malloc array");
    for (size_t i = 0; i < this->_size; i++) {
        va_copy(new_args, *args);
        this->_tab[i] = va_new(this->_type, &new_args);
        va_end(new_args);
    }
}
